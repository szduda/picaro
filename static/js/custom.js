...
*/
const accInput = document.querySelector('#accelerationInput');

accInput.addEventListener('input', (e) => {
  console.log('acc:', e.target.value);
  sendMessage(e.target.value > 29 ? 'forward' : e.target.value < -29 ? 'reverse' : 'stop');
  sendMessage("speed : " + Math.abs(e.target.value));
});
